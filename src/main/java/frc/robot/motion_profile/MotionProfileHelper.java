 ...
 * Although this code uses a TalonSRX, nowhere in this module do we changeMode() or call set() to change the output.
...
public class MotionProfileHelper {
	...
	private boolean _bHold=false;

	/**
	 * Since the TalonSRX.set() routine is mode specific, deduce what we want
	 ...
	 * to do this, just call _talonLeft.processMotionProfileBuffer() in your teleop loop.
	...
	class PeriodicRunnable implements java.lang.Runnable {
	    public void run() {  _talonLeft.processMotionProfileBuffer();    }
	}
	...
	public MotionProfileHelper(TalonSRX talon, double[][] profile, int total, boolean invert) {
        _talonLeft = talon;
        _total=total;
		_profile=profile;
		inverted=invert;
		...
		if (_talonLeft.getControlMode() != ControlMode.MotionProfile) {
			...
			switch (_stateLeft) {
				...
				case 1: /*
					...
					if (_statusLeft.btmBufferCnt > kMinPointsInTalon) {
				...
				case 2: /* check the status of the MP */
					 ...
					 */
					if (_statusLeft.isUnderrun == false) {
						_loopTimeout = kNumLoopsTimeout;
					}
				...
				//TODO: see comment
					...
					if (_statusLeft.activePointValid && _statusLeft.isLast){//_statusLeft.activePoint.isLastPoint) {
						/*
	...
	private void startFilling(double[][] profile, int totalCnt) {
		...
		if (_statusLeft.hasUnderrun) {
			 ...
			 */
			_talonLeft.clearMotionProfileHasUnderrun();
		}
		/*
		...
		for (int i = 0; i < totalCnt; ++i) {
			...
			int multiplier = 1;
			if(inverted)multiplier=-1;
			point.position = profile[i][0]*multiplier;
			point.velocity = profile[i][1] /10*multiplier;
			point.timeDur = (int) profile[i][2]; //TODO was timeDurMS
			...
			_talonLeft.pushMotionProfileTrajectory(point);
		}
	}
